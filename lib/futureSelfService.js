...
export const VALID_BACKGROUND_IMAGES = [
  // Core backgrounds
...
const IMAGE_KEYWORDS = {
  ...
  "/images/skyscapes/aurora-coastal-beacon.png":
    ["coast", "beacon", "sea", "ocean", "lighthouse", "shore", "signal", "horizon"],
  "/images/skyscapes/aurora-lake-sunset.png":
    ["lake", "sunset", "peaceful", "reflection", "calm", "water", "twilight", "quiet"],
  "/images/skyscapes/aurora-mountain-storm.png":
    ["mountain", "storm", "danger", "conflict", "tension", "weather", "peak", "high ground"],
  "/images/skyscapes/aurora-winter-forest.png":
    ["winter", "snow", "cold", "forest", "isolation", "frozen", "ice", "frost"],
  "/images/outcomes/ending-balanced-future.png":
...
// Avoids images already used in this game run; falls back gracefully if all 13 are used.
export function pickBestImageFromPrompt(imagePrompt, usedImageUrls = []) {
  const available = VALID_BACKGROUND_IMAGES.filter(
    (url) => !usedImageUrls.includes(url),
  );
  // If every image has been used (a run longer than 13 turns), allow repeats.
  ...
  for (const url of pool) {
    const keywords = IMAGE_KEYWORDS[url] ?? [];
...
- imagePrompt: 1-2 sentence scene description. Match scene mood to one of these visual types: bunker/shelter, desert highway, forest/safe zone, radio tower, ruined city/urban ruins, coastal beacon/sea, lake sunset, mountain storm, winter forest, balanced future community, broken future void, chaotic future fire, rebuilding future scaffold.
...
export function normalizeGeneratedScenario(rawScenario, usedImageUrls = []) {
  ...
  const imageUrl = pickBestImageFromPrompt(rawScenario.imagePrompt, usedImageUrls);
