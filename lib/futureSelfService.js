...
function getImageCategory(url) {
  if (url.includes("/images/scenarios/")) return "avatar";
  if (url.includes("/images/backgrounds/")) return "background";
  if (url.includes("/images/endings/")) return "ending";
  return "other";
}

function buildSceneMatchSegments(input) {
  ...
  return [
    {
      text: Array.isArray(input.imageKeywords)
        ? input.imageKeywords.join(" ")
        : input.imageKeywords,
      weight: 5,
    },
    { text: input.imagePrompt, weight: 3 },
    { text: input.title, weight: 2.25 },
    { text: input.setting ?? input.message, weight: 2 },
    { text: input.message, weight: 1 },
    { text: input.futureMsg ?? input.futureSelfMessage, weight: 0.75 },
  ].filter((segment) => typeof segment.text === "string" && segment.text.trim());
}

function scoreImagePromptMatch(input, url) {
  const slug = url
    .split("/")
    .pop()
    ?.replace(/\.[^.]+$/, "")
  ...
  const segments = buildSceneMatchSegments(input);
  let score = getImageCategory(url) === "avatar" ? 4 : 0;

  if (input && typeof input === "object" && input.imageUrl === url) {
    score += 100;
  }

  if (getImageCategory(url) === "ending") {
    score -= 8;
  }

  for (const { text, weight } of segments) {
    const prompt = normalizeMatchText(text);
    if (!prompt) continue;

    if (prompt.includes(slugPhrase)) score += 18 * weight;
    ...
    for (const keyword of getImageKeywords(url)) {
      ...
      if (normalized.includes(" ")) {
        if (prompt.includes(normalized)) score += 8 * weight;
      } else if (new RegExp(`\\b${escapeRegExp(normalized)}\\b`).test(prompt)) {
        score += 1.5 * weight;
      }
    }
  }
...
export function pickBestImageFromPrompt(imagePrompt, usedImageUrls = [], avatarId = null) {
  ...
  if (
    !imagePrompt ||
    (typeof imagePrompt !== "string" && typeof imagePrompt !== "object")
  ) {
    return selectableImages.find((url) => !usedImageUrls.includes(url)) ?? selectableImages[0];
  }

  let bestUrl = selectableImages[0];
  let bestScore = -1;

  for (const url of selectableImages) {
    const repeatPenalty = usedImageUrls.includes(url) ? 14 : 0;
    const score = scoreImagePromptMatch(imagePrompt, url) - repeatPenalty;
    if (score > bestScore) {
      bestScore = score;
      bestUrl = url;
    }
