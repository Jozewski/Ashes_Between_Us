...
// Return the set of imageUrls already used for scenarios in this game run.
// Used to prevent the same background appearing twice across 10 turns.
async function getUsedImageUrlsForRun(avatarId, runId) {
  try {
    const where = runId ? { avatarId, runId } : { avatarId };
    ...
    const scenarioIds = attempts.map((a) => a.scenarioId).filter(Boolean);
    if (!scenarioIds.length) return [];
    const scenarios = await prisma.scenario.findMany({
      where: { id: { in: scenarioIds } },
      select: { imageUrl: true },
    });
    return [...new Set(scenarios.map((s) => s.imageUrl).filter(Boolean))];
  } catch {
    return [];
  }
}
...
function pickBackgroundForStats(stats, usedImageUrls = []) {
  ...
  const available = VALID_BACKGROUND_IMAGES.filter(
    (url) => !usedImageUrls.includes(url),
  );
  const pool = available.length > 0 ? available : VALID_BACKGROUND_IMAGES;
  const match = pool.find((url) =>
    url.includes(preferred[pressure] ?? "ruined-city"),
  );
  return match ?? pool[0] ?? "/images/backgrounds/ruined-city.png";
}

function pickFallbackScene({ avatarId, stats, attemptCount, name, memoryLine }) {
  const scenes = [
    {
      title: "The Ration Bell",
      setting:
        `${name} stands in a depot chapel where the ration bell rings for people not on the list. ` +
        `A clerk hides extra tins, guards block the doors, and ${memoryLine}`,
      message:
        "Food can become law faster than anyone admits. The next order will decide whether hunger obeys a ledger or a conscience.",
    },
    {
      title: "The Floodgate Oath",
      setting:
        `${name} reaches a cracked floodgate as two settlements argue over which valley drowns. ` +
        `Children wait uphill with buckets, mechanics hold the only tools, and ${memoryLine}`,
      message:
        "Water is not neutral anymore. Every fix saves one future while teaching another that it was expendable.",
    },
    {
      title: "The Static Trial",
      setting:
        `${name} enters a radio room where a captured broadcaster is accused of inventing tomorrow's panic. ` +
        `The crowd wants a verdict before the signal returns, and ${memoryLine}`,
      message:
        "Truth arrives damaged after the collapse. Punishing a lie too quickly can become another kind of broadcast.",
    },
    {
      title: "The Snowline Convoy",
      setting:
        `${name} watches a winter convoy stall below the pass while refugees, fuel carriers, and armed scouts accuse each other of theft. ` +
        `${memoryLine}`,
      message:
        "Cold turns every delay into a body count. Moving now, waiting, and choosing who rides all carry a different wound.",
    },
  ];
  const signal =
    attemptCount + stats.hope * 2 + stats.trust * 3 + stats.chaos * 5 + stats.humanity * 7;
  const avatarFrame = pickAvatarSceneFrame(avatarId, signal);
  if (avatarFrame) {
    return {
      title: avatarFrame
        .split(" ")
        ...
        .join(" "),
      setting:
        `${name} faces ${avatarFrame}. ${memoryLine}`,
      message:
        "This decision belongs to your role before it belongs to the group. The right answer must use the instincts that kept you alive.",
    };
  }

  return scenes[Math.abs(signal) % scenes.length];
}

function getLocalFallbackChoices({ avatarId, pressure, attemptCount, stats }) {
  ...
  return [
    ...getAvatarFallbackChoices(avatarId),
    pressureChoice[pressure] ?? pressureChoice.hope,
    ...rotated,
  ].slice(0, 6);
}
...
function buildLocalGeneratedFallbackScenario({
  ...
  return {
    title: `${scene.title ?? pressureTitle[pressure] ?? "The Broken Crossroad"} (${turnLabel})`,
    setting: scene.setting,
    message:
      `${scene.message} A ${roleLens} instinct says the obvious answer is probably bait.`,
    futureMsg,
    futureSelfMessage: futureMsg,
    imageUrl: pickBackgroundForStats(stats, usedImageUrls),
    consequences: {
      summary:
    ...
    choices: getLocalFallbackChoices({
      avatarId: avatar?.id,
      pressure,
      attemptCount,
      stats,
...
export async function getAttemptHistory(runId) {
  ...
  const scenarioIds = [
    ...new Set(attempts.map((attempt) => attempt.scenarioId).filter(Boolean)),
  ];
  ...
  return attempts.map((attempt) => {
    ...
    return {
      ...
      scenarioFutureMsg: scenario.futureSelfMessage ?? scenario.futureMsg,
...
export async function generateScenario({
  ...
  // Collect images already shown this run so no background repeats across 10 turns.
  const usedImageUrls = await getUsedImageUrlsForRun(avatarId, runId);
  ...
  if (allowSeededScenarioFallback()) {
    try {
      ...
      if (seededScenario) {
        // Reassign the image if it was already shown in this run.
        const finalScenario =
          usedImageUrls.includes(seededScenario.imageUrl)
            ? {
                ...seededScenario,
                imageUrl: pickBestImageFromPrompt(
                  seededScenario.title,
                  usedImageUrls,
                  avatarId,
                ),
              }
